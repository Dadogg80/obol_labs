...
describe("Optimized", function () {
  ...
  async function deployOptimizedFixture() {
      ...
      const OptimizedContract = await ethers.getContractFactory("Optimized");
      const optimized = await OptimizedContract.deploy(contributors, { value: initialBalance });
    ...
    describe("Optimized Contract", function () {
        ...
        it("should successfully distribute funds after 'distributeTime'", async function () {
            ...
            advanceTime();
            const tx = await optimized.distribute();
